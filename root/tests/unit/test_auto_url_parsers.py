...
class TestAutoUrlParsers(unittest.TestCase):
    ...
    def test_url_pattern_matching(self, input_url, expected_site, expected_captured_url, description):
    ...
    @parameterized.expand([
        SiteIdentifierTestCase("www.fanfiction.net", "fanfiction.net", "fanfiction"),
        SiteIdentifierTestCase("archiveofourown.org", "archiveofourown.org", "archiveofourown"),
        SiteIdentifierTestCase("forums.spacebattles.com", "spacebattles.com", "spacebattles"),
        SiteIdentifierTestCase("forums.sufficientvelocity.com", "sufficientvelocity.com", "sufficientvelocity"),
        SiteIdentifierTestCase("forum.questionablequesting.com", "questionablequesting.com", "questionablequesting"),
        SiteIdentifierTestCase("www.royalroad.com", "royalroad.com", "royalroad"),
        SiteIdentifierTestCase("www.fictionpress.com", "fictionpress.com", "fictionpress"),
    ...
    def test_url_parsers_generation(self):
        ...
        self.assertGreater(len(self.url_parsers), 50, "Should have generated many URL parsers")
        
        # Should contain key sites with algorithmic identifiers
        expected_sites = ["fanfiction", "archiveofourown", "spacebattles", "sufficientvelocity", "questionablequesting", "royalroad", "fictionpress", "other"]
        ...
        for site_id, (pattern, prefix) in self.url_parsers.items():
            ...
            self.assertIsInstance(prefix, str, f"Prefix for {site_id} should be a string")
        ...
        for site_name, example_urls in self.fanficfare_examples:
            ...
            for site_id, (pattern, prefix) in self.url_parsers.items():
                try:
                    ...
                    self.fail(f"Pattern for {site_id} failed to match {example_url}: {e}")
            ...
            try:
                ...
                for test_url in test_urls:
                    try:
                        ...
                        compilation_failures.append((site_id, f"Failed to match {test_url}: {e}"))
                        
            except Exception as e:
                compilation_failures.append((site_id, f"Pattern compilation failed: {e}"))
    ...
    def test_url_parser_coverage(self):
        ...
        for site_id in self.url_parsers.keys():
            ...
            elif site_id in ['spacebattles', 'sufficientvelocity', 'questionablequesting'] or 'forum' in site_id:
                site_types['forum_sites'] += 1
            else:
                site_types['other_sites'] += 1
        
        # Should have representation from different types of sites
        self.assertGreater(site_types['archive_sites'], 0, "Should have archive-type sites")
        self.assertGreater(site_types['forum_sites'], 0, "Should have forum-type sites")
        self.assertGreater(site_types['other_sites'], 10, "Should have many other sites")  # Includes 'other' fallback
    ...
    def test_pattern_robustness(self, input_url, expected_site, description):
        ...
        self.assertEqual(result.site, expected_site, 
    ...
    def test_url_normalization_consistency(self, input_urls, expected_base, description):
        ...
        self.assertEqual(len(unique_normalized), 1, 
        ...
        self.assertTrue(normalized.startswith(expected_base),
    ...
    def test_all_patterns_compile_successfully(self):
        ...
        for site, (pattern, prefix) in self.url_parsers.items():
            try:
                ...
                self.assertIsInstance(pattern, re.Pattern, 
                                    f"Pattern for {site} should be compiled regex")
                
                # Test that prefix is a string
                self.assertIsInstance(prefix, str, 
                                    f"Prefix for {site} should be string")
        ...
        self.assertEqual(len(failed_patterns), 0, 
                        f"Failed patterns: {failed_patterns[:10]}")  # Show first 10 failures
    ...
    def test_regex_pattern_security(self):
        ...
        self.assertEqual(len(vulnerable_patterns), 0, 
    ...
    def test_comprehensive_site_coverage(self):
        ...
        self.assertGreater(len(found_categories['archives']), 0, 
                          "Should have archive-type sites")
        self.assertGreater(len(found_categories['forums']), 0, 
                          "Should have forum-type sites")
